...
export default function FacultadesForm({ facultad, sedes, errors: initialErrors }: Props) {
  const isEditing = !!facultad;
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState<Record<string, string>>(initialErrors || {});
  
  const [formData, setFormData] = useState({
    ID_Sede: facultad?.ID_Sede?.toString() || (sedes[0]?.ID_Sede?.toString() || ''),
      ...
      <div className="page-header">
        <div className="page-title">
          ...
          <p className="page-subtitle">
            {isEditing ? 'Modifica los datos de la facultad' : 'Registra una nueva facultad'}
          </p>
        </div>
      </div>
          ...
          <div className="form-grid">
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={handleChange}
                className={errors.Nombre_Facultad ? 'input-error' : ''}
                required
              />
              {errors.Nombre_Facultad && <span className="error-message">{errors.Nombre_Facultad}</span>}
            </div>

            <div className="form-group">
              <label htmlFor="Conmutador_Facultad">Conmutador</label>
              <input
                type="text"
            ...
            <div className="form-group">
              <label htmlFor="Extension_Facultad">Extensión</label>
              <input
                type="text"
