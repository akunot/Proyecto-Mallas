...
export default function ProgramasForm({ programa, facultades }: Props) {
  ...
  const { data, setData, post, put, processing, errors } = useForm({
    ID_Facultad: programa?.ID_Facultad?.toString() || '',
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
            ...
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="lg:col-span-1">
                ...
                <select 
                  value={data.ID_Facultad} 
                  onChange={e => setData('ID_Facultad', e.target.value)}
                  className={`w-full mt-1 px-4 py-3 bg-slate-50 border-2 rounded-xl focus:ring-4 focus:ring-blue-100 transition-all ${errors.ID_Facultad ? 'border-rose-300' : 'border-transparent focus:border-blue-500'}`}
                >
                  <option value="">Seleccionar facultad...</option>
                  {facultades.map(f => <option key={f.ID_Facultad} value={f.ID_Facultad}>{f.Nombre_Facultad}</option>)}
                </select>
              </div>
