...
export default function Sedes({ sedes: initialSedes }: Props) {
  ...
  const initialMeta = initialSedes?.meta || { current_page: 1, total: 0, per_page: 20, last_page: 1 };
  ...
  const handleSearch = async (search: string, page: number = 1) => {
    ...
    try {
      if (!search) {
        ...
        const filtered = initialData.filter(sede => 
          sede.Nombre_Sede.toLowerCase().includes(search.toLowerCase()) ||
          sede.Ciudad_Sede?.toLowerCase().includes(search.toLowerCase())
        );
        setData(filtered);
    ...
    <MainLayout>
      ...
      <div className="page-header">
        ...
        <div className="page-actions">
          <button className="btn-primary" onClick={() => router.visit('/sedes/create')}>
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
